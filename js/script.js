...
(function() {
	...
	const apiKey = "57d974e138278c3b9fdd0e24652d3b31"; // Replace "APIKEY" with your own API key; otherwise, your HTTP request will not work
	const activities = {
	...
	document.querySelector('.forecast-button').addEventListener('click', function(e) {
		...
		fetch(url + location + '&appid=' + apiKey).then(response => response.json())
	...
	document.querySelectorAll('.options div').forEach(el => el.addEventListener('click', updateActivityList, false))
	...
	function updateUISuccess(response) {
		...
		// conditionsPara.textContent = state.degCInt + '\U00B0 C / ' + state.degFInt + '\U00B0 F'
	...
	function updateUIFailure() {
		...
		document.querySelector('.conditions').textContent = "Weather Information not Available"
	}
})();